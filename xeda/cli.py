...
class ListDesignsAction(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        try:
            xp = load_xedaproject(Path(namespace.xedaproject))
...
def get_main_argparser():
    ...
    parser = argparse.ArgumentParser(
        prog=__package__,
        description=f'{__package__}: Cross-EDA abstraction and automation. Version: {xeda_app.__version__}',
    ...
    parser.add_argument(
        '--version',  action='version', version=f'%(prog)s {xeda_app.__version__}', help='Print version information and exit',
    )
    ...
    try:
        with open(project_file) as f:
            ...
            if ext == '.json':
                ...
                raise ValueError(f"xedaproject: {project_file} has unknown extension {ext}. Currently supported formats are TOML (.toml) and JSON (.json)")
    except FileNotFoundError as e:
        logger.error(f'Cannot open project file: {project_file}. Please run from the project directory with xedaproject.toml or specify the correct path using the --xedaproject flag')
        raise e
...
def main(args=None):
    ...
    xeda_app.run_xeda(xeda_project, parsed_args)
